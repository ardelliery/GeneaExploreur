window.RelationModule = {
  ...
  resetPersonB() {
    console.log("[Relation] Reset de la sélection B");

    // 1. On vide la donnée technique
    this.selectedB = null;

    // 2. On nettoie l'interface utilisateur (Champs texte et zones de résultats)
    const inputB = document.getElementById("rel-input-b");
    if (inputB) {
      inputB.value = "";
      inputB.focus(); // Optionnel : redonne le focus pour chercher une autre personne
    }
    ...
    // 3. On déclenche la mise à jour du graphe
    if (window.NetworkModule) {
      // Comme selectedB est désormais null, le render() du NetworkModule
      // va automatiquement ignorer le "Mode 3" (Chemin) et repasser
      // en "Mode 2" (Lignée de A) ou "Mode 1" (Global).
  ...
  executeSearch() {
    ...
    if (path) {
      console.log("[Relation] Chemin trouvé :", path);
      zone.style.display = "block";
      this.renderPath(path, list);

      // LANCEMENT DE L'AFFICHAGE DANS LE GRAPHE
      if (window.NetworkModule) {
        console.log("[Relation] Affichage du chemin dans le NetworkModule...");
        window.NetworkModule.highlightPath(path);
      }
    } else {
      alert("Aucun lien de parenté trouvé entre ces deux personnes.");
    }
  },
  ...
  analyzeRelation(path) {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      const link = App.fullData.links.find(
        (l) =>
          (l.source == current && l.target == next) ||
          (l.source == next && l.target == current),
      );
      ...
      if (link) {
        ...
        if (link.type === "parent") {
          if (link.target == next)
            downCount++; // On descend vers l'enfant
    ...
    console.log(
      `[Analysis] Up:${upCount}, Down:${downCount}, Alliance:${hasSpouse} -> Result: ${type}`,
    );
  ...
  renderPath(path, container) {
    if (!path) {
      container.innerHTML =
        "<div style='padding:20px; color:red; text-align:center;'>Aucun lien trouvé.</div>";
      return;
    }
